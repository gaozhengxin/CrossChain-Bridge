...
func (b *Bridge) StartChainTransactionScanJob() {
	...
	for {
		...
		for h := stable; h <= latest; {
			...
			// get msgs from all blocks, add to a map
			msgs := make(map[string]int)
			for _, block := range tipset.Blocks() {
				bms := b.GetBlockMessages(block.Cid().String())
				for _, msgid := range bms {
					msgs[msgid] = 1
				}
			}
			for msg := range msgs {
...
func (b *Bridge) quickSyncRange(ctx context.Context, idx, start, end uint64, wg *sync.WaitGroup) {
	...
	for h := start; h < end; {
		...
		log.Tracef("[scanchain] id=%v scanned %v block, height=%v hash=%v txs=%v", idx, chainName, h, tipsetStr, len(msgs))
		h++
	}
